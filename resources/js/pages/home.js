...
async function createSelectBox() {
    ...
    for(let i = 0; i < data.length; i++) {
        ...
        <li>
            <input class="province" type="checkbox" name="province[]" value="${data[i].code}" id="">
            <p>${data[i].name}</p>
...
async function createDistrictSelectBox() {
    ...
    data.forEach((dataItem, index) => {
        ...
        //         <li>
        //             <input type="checkbox" name="district[]" value="all_${dataItem.code}" id="" province_code="${dataItem.code}" class="all-district">
        //             <p>Tất cả</p>
        //         </li>
        //     `;
        districtSelectBoxHtml += `
            <div code="${dataItem.code}" style="display: none;">
                <li>
                    <input type="checkbox" id="" province_code="${dataItem.code}" class="all-district">
                    <p>Tất cả</p>
                </li>
            `;
        for (var i = 0; i < dataItem.districts.length; i++) {
            districtSelectBoxHtml += `
            <li>
                <input type="checkbox" name="district[]" value="${dataItem.districts[i].code}" id="" province_code="${dataItem.code}">
...
// // khi chon province thi se chon all district
document.querySelector('.search__location--province ul').addEventListener('change', function(event) {
    const target = event.target;
    if (target.classList.contains('province') && target.type === 'checkbox') {
        if (target.checked) {
            console.log('đã chọn');
            let id = target.getAttribute('value');
            allDistrictCheckedToTrue(id);
        } else {
            console.log('bỏ chọn');
            let id = target.getAttribute('value');
            allDistrictCheckedToFalse(id);
        }
    }
});
