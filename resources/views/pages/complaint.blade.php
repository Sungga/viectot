...
@section('content')
    <section class="report">
        <div class="grid-container">
            <form method="POST" action="{{ route('complaint', ['id_post' => $post->id]) }}" class="report__container" enctype="multipart/form-data">
                @csrf
                <h1>Khiếu nại</h1>
                <p class="report__item">Khiếu nại bài viết: {{ $post->title }} #{{ $post->id }}</p>
