            ...
            <div class="dashboard__left--item">
                <div class="dashboard__left--img">
                    <a href="{{ route('post.show', ['id_post' => $post->id]) }}">
                        <img src="{{ asset('storage/uploads/'.$post->branch->company->logo) }}" alt="">
                    </a>
                </div>
                <div class="dashboard__left--about">
                    <p class="dashboard__left--name"><a href="{{ route('post.show', ['id_post' => $post->id]) }}">{{ $post->title }}</a></p>
                    <p class="dashboard__left--company"><a href="{{ route('branch.show', ['id_branch' => $post->branch->id]) }}">{{ $post->branch->name }}</a></p>
                    <p class="dasboard__left--location">{{ $post->branch->branchProvince->name }}</p>
                </div>
            </div>        
        @endforeach
    </div>
...
<div class="dashboard__right">
    <div class="dashboard__right--top">
        <div class="dashboard__rigthTop--item">
            <p class="dashboard__rightTop--quantity">{{ $totalJobToday }}</p>
            <p class="dashboard__rightTop--name">Việc làm mới hôm nay</p>
        </div>
        <div class="dashboard__rigthTop--item">
            <p class="dashboard__rightTop--quantity">{{ $totalJob }}</p>
            <p class="dashboard__rightTop--name">Việc làm đang tuyển</p>
        </div>
        <div class="dashboard__rigthTop--item">
            <p class="dashboard__rightTop--quantity">{{ $totalBranch }}</p>
